...
#include "openvdb/openvdb.h"
	...
	void RawLam::CtLog::readVDB(const std::string filename)
	{
	...
	std::vector<ValueT> RawLam::CtLog::evaluate(std::vector<Eigen::Vector3f> coords, int sample_type)
	{
	...
	void RawLam::CtLog::filter(int width, int iterations, int type)
	{
	...
	void RawLam::CtLog::offset(ValueT amount)
	{
		openvdb::tools::Filter<GridType> tool(*m_grid);
		tool.offset(amount);
	}

	openvdb::math::CoordBBox RawLam::CtLog::bounding_box()
	{
		return m_grid->evalActiveVoxelBoundingBox();
	}

	Eigen::Matrix4d RawLam::CtLog::get_transform()
	{
	...
	void RawLam::CtLog::set_transform(Eigen::Matrix4d mat)
	{
	...
	void RawLam::CtLog::to_mesh(float isovalue, std::vector<Eigen::Vector3f>& verts, std::vector<Eigen::Vector4i>& faces)
	{
	...
	RawLam::CtLog* RawLam::CtLog::resample(float scale)
	{
